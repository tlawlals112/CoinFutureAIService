...
class ClaudeEngine:
    ...
    def __init__(self):
        ...
        self.config = LLM_CONFIG['claude']
        self.logger = logger.bind(name="claude_engine")
...
"""
    ...
    def analyze_market(self, market_data: Dict) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"Claude 분석 완료 - 소요시간: {processing_time:.3f}초")
            
            # LLM 분석 결과 저장
            self._save_analysis_result(market_data['symbol'], prompt, response.content[0].text, processing_time)
            
            return result
            
        except Exception as e:
            self.logger.error(f"Claude 분석 오류: {e}")
